...
var configure = function (config) {
	...
	// Load Settings File
	var settingsFile = path.resolve(__dirname, 'http.conf');
	...
	if (settings.accessLog !== false) {
		settings.accessLog = path.resolve(__dirname, settings.accessLog);
	}
	
	// Error logging
	if (settings.errorLog !== false) {
		process.on('uncaughtException', function(err) {
			logging(path.resolve(__dirname, settings.errorLog), {
				date	: getDateRFC1123(),
				error	: err
			});
		});
